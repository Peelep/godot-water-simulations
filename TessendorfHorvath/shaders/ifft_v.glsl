...
/*
    This compute shader computes the vertical steps in Cooley-Tukey FFT
...
void main()
{
  ...
  vec4 data = imageLoad(twiddle_data, ivec2(params.loop_step, id.y));
  ivec2 input_indices = ivec2(int(data.b), int(data.a));
  vec2 twiddle_vec = vec2(data.r, -data.g);

	if (params.ping_pong == 1) {
    vec2 load_x = imageLoad(buffer_0, ivec2(id.x, input_indices.x)).xy;
    vec2 load_y = imageLoad(buffer_0, ivec2(id.x, input_indices.y)).xy;
    vec2 result = load_x + ComplexMult(twiddle_vec, load_y);
    imageStore(buffer_1, id.xy, vec4(result.r, result.g, 0.0, 0.0));
	}
	else {
    vec2 load_x = imageLoad(buffer_1, ivec2(id.x, input_indices.x)).xy;
    vec2 load_y = imageLoad(buffer_1, ivec2(id.x, input_indices.y)).xy;
    vec2 result = load_x + ComplexMult(twiddle_vec, load_y);
